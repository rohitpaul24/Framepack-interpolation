...
app = FastAPI(title="Video Loop Generation Server", version="1.0.0")
...
async def health_check():
    ...
    if not models:
        return JSONResponse(
            status_code=503,
            content={"status": "not_ready", "message": "Models are still loading"}
        )
...
async def process_video(file: UploadFile = File(...)):
    ...
    try:
        ...
        result = process_video_multi_loops_api(upload_path)
...
def process_video_multi_loops_api(input_video_path):
    """
    ...
    if num_seconds < 1:
        raise ValueError("Video must be at least 1 second long")
    ...
    # Configuration - matches user's working demo_gradio settings
    config = {
        'prompt': 'silent lips',         # User's working prompt
        'n_prompt': '',
        'seed': 42,
        'total_second_length': 0.5,      # User's working setting
        'latent_window_size': 7,         # User's working setting
        'steps': 10,                     # User's working setting
        'cfg': 1.0,
        'gs': 10.0,
        'rs': 0.0,
        'gpu_memory_preservation': 6.0,  # Match demo_gradio.py default
        'use_teacache': True,
        'mp4_crf': 16
    }
    ...
    for sec in range(1, num_seconds):
        print(f"\nProcessing {sec}s mark ({sec}/{num_seconds-1})")
    ...
    return {
        ...
        "outputs": {
            ...
            "loopable_videos": loopable_videos
        }
    }
