...
server_config = {
    'prompt': 'silent lips, static camera and lighting',
    'n_prompt': '',
    'seed': 42,
...
# Replace these two functions in server_loop_fixed.py (lines 720-763)

def trim_video_to_timestamp(input_video, output_path, end_timestamp, fps):
    """
    Trim video from start to just before the specified timestamp using frame-based approach.
    
    This is more reliable than ffmpeg time-based trimming which can have rounding errors.
    """
    print(f"  Trimming video to {end_timestamp}s: {os.path.basename(output_path)}")
    
    # Calculate exact frame to stop at (exclusive)
    ...
    print(f"    Trimmed to {frames_written} frames ({actual_duration:.2f}s)")
...
def concatenate_videos(video1_path, video2_path, output_path):
    """
    Concatenate two videos using frame-by-frame approach for reliability.
    
    This avoids ffmpeg concat issues with:
    ...
    print(f"    Concatenated {len(frames)} frames at {fps} FPS")
